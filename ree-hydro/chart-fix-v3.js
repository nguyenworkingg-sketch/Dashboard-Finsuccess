...
const haveData=()=>typeof R!=='undefined'&&typeof M!=='undefined'&&typeof P!=='undefined';
const rowsFor=name=>R.filter(x=>x[2]===name).sort((a,b)=>a[0].localeCompare(b[0]));
...
function redrawReservoir(){
 ...
 if(!sel||!fc||!qc||typeof Chart==='undefined')return;
 ...
 new Chart(fc,{type:'line',data:{labels,datasets:[
  {label:'Mức đầy hồ cùng kỳ 2025',data:d.map(x=>full25(x[0],x[2])),borderColor:YELLOW,backgroundColor:YELLOW,tension:.25,pointRadius:3},
  {label:'Mức đầy hồ 2026',data:d.map(x=>x[6]==null?null:x[6]*100),borderColor:BLUE,backgroundColor:BLUE,tension:.25,pointRadius:3}
 ]},options:opts('%')});
...
function redrawPlant(){
 ...
 if(!sel||!fc||!qc||typeof Chart==='undefined')return;
 const name=sel.value,d=M.filter(x=>x[1]===name).sort((a,b)=>a[0].localeCompare(b[0]));if(!d.length)return;
 const meta=P.find(x=>x.plant===name),labels=d.map(x=>'T'+(+x[0].slice(5))+'/26');
 Chart.getChart(qc)?.destroy();
 new Chart(qc,{data:{labels,datasets:[
  {type:'bar',label:'Lượng nước về hồ cùng kỳ 2025',data:d.map(x=>x[11]),backgroundColor:YELLOW,borderColor:YELLOW,order:1},
  {type:'bar',label:'Lượng nước về hồ 2026',data:d.map(x=>x[7]),backgroundColor:BLUE2,borderColor:BLUE2,order:2},
 ...
 Chart.getChart(fc)?.destroy();
 new Chart(fc,{type:'line',data:{labels,datasets:[
  {label:'Mức đầy hồ cùng kỳ 2025',data:d.map(x=>plantFull25(meta,x[0])),borderColor:YELLOW,backgroundColor:YELLOW,tension:.25,pointRadius:3},
  {label:'Mức đầy hồ 2026',data:d.map(x=>x[6]==null?null:x[6]*100),borderColor:BLUE,backgroundColor:BLUE,tension:.25,pointRadius:3}
 ]},options:opts('%')});
}
function redrawAll(){redrawReservoir();redrawPlant()}
function boot(){
 setTimeout(redrawAll,100);setTimeout(redrawAll,500);setTimeout(redrawAll,1200);
 document.getElementById('reservoirSelect')?.addEventListener('change',()=>setTimeout(redrawReservoir,100));
 document.getElementById('plantSelect')?.addEventListener('change',()=>setTimeout(redrawPlant,100));
 document.addEventListener('click',e=>{if(e.target.closest?.('.pp-item'))setTimeout(redrawPlant,160)},true);
 let lastP=document.getElementById('plantSelect')?.value||'',lastR=document.getElementById('reservoirSelect')?.value||'';
 setInterval(()=>{const p=document.getElementById('plantSelect')?.value||'',r=document.getElementById('reservoirSelect')?.value||'';if(p&&p!==lastP){lastP=p;setTimeout(redrawPlant,100)}if(r&&r!==lastR){lastR=r;setTimeout(redrawReservoir,100)}},200);
 console.log('REE chart-fix-v3 active',{fullness2025:!!window.REE_FULLNESS_2025,plants:typeof P!=='undefined'?P.length:null,monthly:typeof M!=='undefined'?M.length:null,reservoir:typeof R!=='undefined'?R.length:null});
}
